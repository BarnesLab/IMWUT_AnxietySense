# AnxietySense

Codes for our paper "AnxietySense: A Multimodal Smartwatch Framework for Predicting State Anxiety via Weakly Supervised Meta-Learning"

`Root_anxiety.ipynb`: Contains all codes (starting from generating the images from sensing data) of the weakly labeled framework to generate predicted probabilities.

`Root_Model Evaluation.ipynb`: Contains the personalized meta-learner responsible for making the final predictions.
